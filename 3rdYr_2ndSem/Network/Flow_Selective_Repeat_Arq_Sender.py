        ...
        while True:
            if self.ack_id == len(self.frames)-1:
                ...
                for send_id in range(self.ack_id, self.ack_id+self.frame_len, 1):
                    if self.sent_timer[send_id%self.frame_len] == 0:
                        print("Sending Frame : ",self.frames[send_id])
                        self.sender_client.send(self.frames[send_id].encode())
                        self.sent_timer[send_id%self.frame_len] = time.time()
                    elif self.sent_timer[send_id%self.frame_len]!=-1 and (time.time()-self.sent_timer[send_id%self.frame_len]>self.timer_threshold):
